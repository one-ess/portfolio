//стороняя библиотека для перемещения элементов по дом-дереву в определенные брейкпоинты
import "./js/dynamicAdapt.js";
...
//на 992px accordionsWrapper добавляется класс projects__info_accordion т.к он будет являтся обёрткой для аккардеона
  ...
  accordionWrapper.addEventListener("click", (e) => {
    if (e.currentTarget.classList.contains("projects__info_accordion")) {
      ...
      if (e.currentTarget.querySelector(".projects__accordion").classList.contains("projects__accordion_show")) {
        e.currentTarget.querySelector(".projects__accordion").style.maxHeight =
          e.currentTarget.querySelector(".projects__accordion").scrollHeight + `px`;
      } else {
...
tabsWrapper.addEventListener("click", (e) => {
  ...
  if (currentTab.classList.contains("projects__tab")) {
    //Пробегаемся по всем табам и если таб тот на который нажали - добавить ему класс
    tabs.forEach((tab) => {
      if (tab === currentTab) {
    ...
    //Пробегаемся по всем контентам табов и если дата атрибут нажатого таба совпадает с дата атрибутом контента таба - добавить ему класс
